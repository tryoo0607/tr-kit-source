...
| 요청 | 수행 전 읽을 문서 |
|---|---|
| 빠른 캡처·자료 추가·기존 지식 통합 | [operations.md](references/operations.md)의 Capture·Ingest + [schema.md](references/schema.md) |
| 기존 지식 조회·질문 | [operations.md](references/operations.md)의 Query + [retrieval.md](references/retrieval.md) |
| 링크·출처·모순·오래된 내용 점검 | [operations.md](references/operations.md)의 Lint + [schema.md](references/schema.md) |
| repository 초기화·구조 변경 | [schema.md](references/schema.md) 전체 |

새 source를 별도 노트로 요약하는 데서 멈추지 않는다. 관련 Wiki를 먼저 찾아 기존 페이지·교차 링크·충돌 표시를 갱신하고, 새 페이지는 별도 개념이 필요할 때만 만든다.

## 결정적 도구

skill directory를 `<skill>`이라 할 때:

```sh
python3 <skill>/scripts/knowledge_index.py <knowledge-repository>
python3 <skill>/scripts/knowledge_index.py --check <knowledge-repository>
python3 <skill>/scripts/knowledge_lint.py <knowledge-repository>
```
